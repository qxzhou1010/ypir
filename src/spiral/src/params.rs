...
impl Params {
    ...
    pub fn crt_compose_2(&self, x: u64, y: u64) -> u64 {
        ...
        // 为啥感觉 它这里的 crt 重构计算和公式对不上 
        // 注意 mod1_inv_mod0 的含义，就可以对上了 
    ...
    pub fn crt_compose(&self, a: &[u64], idx: usize) -> u64 {
        if self.crt_count == 1 {
            // 如果只有一个模数，那就是它本身，无需重构
            self.crt_compose_1(a[idx])
        } else {
            // 如果有两个模数，那就重构 ，a[idx], a[idx + self.poly_len] 正好是每一列的两个值
            // 将这两个值重构为一个值即可 
            self.crt_compose_2(a[idx], a[idx + self.poly_len])

            // 这样看来，这个实现的参数选择限制得非常死啊，只能有两个模数吗？
        }
    }
