...
pub fn get_fast_expansion_testing_params() -> Params {
    ...
    // r#"******"# 这里本质上还是一个字符串，r# 是防止对一些特殊字符串进行转义, 也就是 这里给定的字符串是什么，就是什么
    ...
    let cfg = r#"
        ...
        'db_item_size': 8192 }
    "#;
    // 简单理解，就是从一个 Json 字符串 凡序列化为一个 Params object 
    // replace 是把上述 &str 中的 单引号 ' 全部替换为 双引号 ' 
    params_from_json(&cfg.replace("'", "\""))
}
...
pub fn params_from_json_obj(v: &Value) -> Params {
    ...
    // 直接上传的意思就是 直接上传 n 个密文，其中第 target_idx 个密文是 E(1) 其余都是 E(0)
    // n 是数据库长度 , 默认为 true 
...
mod test {
    ...
    fn simple() {
        ...
        let param = get_fast_expansion_testing_params();
