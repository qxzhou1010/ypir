...
fn serialize_polymatrix_for_rng(vec: &mut Vec<u8>, a: &PolyMatrixRaw) {
    let offs = a.cols * a.params.poly_len; // skip the first row, 因为第一行不做序列化，而是通过 seed 实例化 rng 恢复出来
    for i in 0..(a.rows - 1) * a.cols * a.params.poly_len {
        vec.extend_from_slice(&u64::to_ne_bytes(a.data[offs + i]));
    }
}
/// not consider rng, just copy all rows
pub fn serialize_polymatrix(vec: &mut Vec<u8>, a: &PolyMatrixRaw) {
    for i in 0..a.rows * a.cols * a.params.poly_len {
        vec.extend_from_slice(&u64::to_ne_bytes(a.data[i]));
    }
}
/// not consider rng, just copy all rows
pub fn deserialize_polymatrix(a: &mut PolyMatrixRaw, data: &[u8]) {
    let all_rows = a.data.as_mut_slice();
    // u64 的字节大小为一个 chunk 
    for (i, chunk) in data.chunks(size_of::<u64>()).enumerate() {
        all_rows[i] = u64::from_ne_bytes(chunk.try_into().unwrap());
    }
}
...
impl<'a> Client<'a> {
    ...
    #[allow(dead_code)]
    pub fn get_sk_reg_full(&self) -> &PolyMatrixRaw<'a> {
        &self.sk_reg_full
    }
    ...
    pub fn generate_query(&self, idx_target: usize) -> Query<'a> {
        ...
        let mut query: Query<'_> = Query::empty();
        // let query_seed = ChaCha20Rng::from_entropy().gen();
        let query_seed = [0 as u8; 32];
        ...
        if params.expand_queries {
            ...
            if params.db_dim_2 == 0 {
                for i in 0..(1 << params.db_dim_1) {
                    // 后面第二个参数条件值是 1，则 self = other
                    // 后面第二个参数条件值是 2，则 self = self
                    // 也就是只有 i == idx_dim0 的时候，data[i] = scale_k
                    // 从多项式的角度解读，就是 构造一个多项式 \lfloor q/p \rfloow * x^{i}
                    // 对应 page-18, Query(qk, idx)中的 1. Encoding the first dimension
    ...
    pub fn decode_response(&self, data: &[u8]) -> Vec<u8> {
        ...
        */
        ...
        // 创建一个文件，如果文件已存在，它将被截断
        let mut file = File::create("result_rust.txt").unwrap();

        // 写入文本到文件
        writeln!(file, "result: {:?}", result.data.as_slice());
...
mod test {
    ...
    fn split_test() {
        ...
        for (idx, value) in v[0..num_in].iter().enumerate() {
            println!("{}: {}", idx, value);
        }

        for (idx, value) in v[num_in..num_out].iter().enumerate() {
            println!("{}: {}", idx, value);
        }

        //  v[num_in..num_out]
        //      .iter_mut()
        //      .enumerate() // 这样传递进去的 i 是多少？
        //      .for_each();
    }
    ...
    #[test]
    fn public_parameters_none() {
        let a: Option<Vec<PolyMatrixNTT>> = None;
        // 当输入是 None 的时候，返回值也是一个 None，而不会 panic
        let b = PublicParameters::from_ntt_alloc_opt_vec(&a);

        // println!("{:?}", b.unwrap()[0].data.as_slice());

        // 如果输入是一个 空的 vec 呢？

        let a: Option<Vec<PolyMatrixNTT>> = Some(Vec::new());
        let b = PublicParameters::from_ntt_alloc_opt_vec(&a);
        // 此时的报错信息比较有意思，不是 unwrap() 上报错的，而是 b.unwrap()[0] 上进行的报错。
        // 此时返回值不是 None，仍然是一个为空的数组。
        // println!("{:?}", b.unwrap()[0].data.as_slice());
    }

    // 原版本的实现 测试少的可怜，这是我自己添加的
    #[test]
    fn encrypt_and_decrypt() {
        ...
        // let m = PolyMatrixRaw::zero(&params, 1, 1);

        let m = PolyMatrixNTT::zero(&params, 1, 1);

        println!("m: \n {:?}", m.as_slice());

        let mut client = Client::init(&params);

        let encrtpted_m = client.encrypt_matrix_reg(&m, &mut rng, &mut rng_pub);

        println!("encrtpted_m: \n {:?}", encrtpted_m.as_slice());

        let decrypted_m = client.decrypt_matrix_reg(&encrtpted_m);
        println!("decrypted_m: \n {:?}", decrypted_m.as_slice());
    }

    #[test]
    fn PolyMatrixRaw_serialize_de() {
        let mut rng = ChaCha20Rng::from_entropy();
        let mut rng_pub = ChaCha20Rng::from_entropy();
        let params = get_params();
        ...
        // get a sk
        let sk = client.get_sk_gsw();

        // serialize
        let mut data = Vec::<u8>::new();
        serialize_polymatrix(&mut data, sk);

        // deserialize

        let mut recover_sk = PolyMatrixRaw::zero(&params, params.get_sk_gsw().0, params.get_sk_gsw().1);
        deserialize_polymatrix(&mut recover_sk, data.as_slice());

        // verify cortness
        assert!(sk.rows == recover_sk.rows);
        assert!(sk.cols == recover_sk.cols);
        let length = sk.data.as_slice().len();
        for i in 0..length {
            assert!(sk.data.as_slice()[i] == recover_sk.data.as_slice()[i]);
        }


        // // 32678
        // println!("data len: {}", data.len());
        // // 2 * 1 * 2048 * 8 = 32678 , one u64 is 8-byte, so mul 8
        // println!(
        // "rows: {}, cols: {}, poly_len: {}, u64 size: {}",
        //     sk.rows, sk.cols, params.poly_len, size_of::<u64>()
        // );
