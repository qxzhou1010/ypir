...
impl<'a> PolyMatrix<'a> for PolyMatrixRaw<'a> {
    ...
    // 在 Java 中 不需要这两个方法，统一用 startIndex 来代替
    fn as_slice(&self) -> &[u64] {
        self.data.as_slice()
    }
    fn as_mut_slice(&mut self) -> &mut [u64] {
    ...
    fn random_rng<T: Rng>(params: &'a Params, rows: usize, cols: usize, rng: &mut T) -> Self {
        // 输出满足给定分布的随机数序列，Java如何实现这一点呢？
        // Standard 是什么分布？在文档中找到，就理解为均匀分布即可 Integers (i32, u32, isize, usize, etc.): Uniformly distributed over all values of the type.
        ...
        for r in 0..rows {
            for c in 0..cols {
                for i in 0..params.poly_len {
                    ...
                    // 暂时全部改成 1 
                    // let val: u64 = 100;
                    // 给定的行和列 上是一个 多项式？也就是 这个矩阵的基本元素是 多项式？
                    // 随机值 对 Q 取模, 然后放在对应位置上 
...
pub fn automorph_poly(params: &Params, res: &mut [u64], a: &[u64], t: usize) {
    ...
    for i in 0..poly_len {
        // 每超过一次 N 就取一次反，例如 num = 1 3 5 7 .. 就对系数取一次反
        // num = 0 2 4 6 8 的时候，系数就不变 
    ...
    SCRATCH.with(|scratch_cell| {
        ...
        for r in 0..a.rows {
            for c in 0..a.cols {
                ...
                // println!("before ntt_inverse, b:\n {:?}", b.as_slice());
                // println!("before ntt_inverse, scratch: \n {:?}", scratch);
                ntt_inverse(params, scratch);
                // println!("after ntt_inverse, b:\n {:?}", b.as_slice());
                // println!("after ntt_inverse, scratch:\n {:?}", scratch);
                // let a_slice = a.as_slice();
                // let b_slice = b.as_slice();
...
mod test {
    ...
    fn full_multiply_correctness() {
        ...
        // println!("m3_ntt: {:?}", m3_ntt.data.as_slice());
